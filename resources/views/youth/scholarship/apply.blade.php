            ...
            <div class="main-card card">
                <div class="card-header-custom">
                    @if (count($scholarships) === 1)
                        <h4 class="text-white"><i class="fas fa-graduation-cap"></i> {{ $scholarships->first()->name }}</h4>
                        <p class="subtitle">Application Form</p>
                    @else
                        <h4 class="text-white"><i class="fas fa-graduation-cap"></i> Scholarship Application</h4>
                        <p class="subtitle">Apply for Educational Financial Assistance</p>
                    @endif
                </div>
                            ...
                            <div class="col-md-11 mx-auto">
                                ...
                                <div class="requirement-section">
                                    <div class="requirement-title">
                                        <i class="fas fa-graduation-cap"></i> Select Scholarship Program
                                    </div>
                                    <div class="requirement-content">
                                        @foreach ($scholarships as $scholarship)
                                            <div class="scholarship-card"
                                                onclick="selectScholarship({{ $scholarship->id }})">
                                                <div class="row align-items-center">
                                                    <div class="col-md-9">
                                                        <h5><i class="fas fa-award text-primary"></i>
                                                            {{ $scholarship->name }}</h5>
                                                        <p class="text-muted mb-2">{{ $scholarship->description }}</p>

                                                        @if ($scholarship->start_date && $scholarship->end_date)
                                                            <small class="text-info">
                                                                <i class="fas fa-calendar-alt"></i>
                                                                Application Period:
                                                                {{ date('M d, Y', strtotime($scholarship->start_date)) }} -
                                                                {{ date('M d, Y', strtotime($scholarship->end_date)) }}
                                                            </small>
                                                        @endif
                                                    </div>
                                                    <div class="col-md-3 text-right">
                                                        <input type="radio" name="scholarship_id"
                                                            value="{{ $scholarship->id }}"
                                                            id="scholarship_{{ $scholarship->id }}"
                                                            class="form-check-input d-none"
                                                            {{ !$isProfileComplete ? 'disabled' : '' }}>
                                                        <label for="scholarship_{{ $scholarship->id }}"
                                                            class="btn btn-outline-custom {{ !$isProfileComplete ? 'disabled' : '' }}">
                                                            <i class="fas fa-check"></i> Select
                                                        </label>
                                                    </div>
                                                </div>

                                                <!-- Dynamic Requirements based on scholarship -->
                                                <div id="requirements_{{ $scholarship->id }}"
                                                    class="scholarship-requirements">
                                                    @include(
                                                        'youth.scholarship.partials.scholarship_requirements',
                                                        [
                                ...
                                <div class="requirement-section">
                                    <div class="requirement-title">
                                        <i class="fas fa-handshake"></i> Terms and Agreement
                                    </div>
                                    <div class="requirement-content">
