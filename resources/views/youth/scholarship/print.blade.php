...
<body>
    ...
    <div class="top_header">
        ...
        <h4>APPLICATION FORM FOR THE AWARD OF <br />
            ...
            <br />
            {{ $data['scholarship']->name ?? 'Scholarship Application' }}
            @if (isset($data['student']) && isset($data['student']->created_at) && !auth()->user())
                <br><small style="color: #666;">(Historical Data Preserved)</small>
            @endif
        </h4>
    </div>
    <div class="top_header_print">
        ...
        <h4>APPLICATION FORM FOR THE AWARD OF <br />
            ...
            {{ $data['scholarship']->name ?? 'Scholarship Application' }}</h4>
    </div>
        ...
        <tbody>
            <tr>
                <td>
                    ...
                    <!-- Application -->
                    @php
                        // Use student data from history if available, otherwise fallback to auth user
                        $user = $data['student'] ?? auth()->user();
                    @endphp
                    ...
                    <div class="table_section education">
                        ...
                        <table cellspacing="0">
                            ...
                            @foreach ($data['user_edu'] ?? [] as $edu)
                                <tr>
                                    <td>{{ $edu->level ?? 'N/A' }}</td>
                                    <td>{{ $edu->institute_university_name ?? 'N/A' }}</td>
                                    <td>{{ $edu->passing_year ?? 'N/A' }}</td>
                                    <!--<td>{{ $edu->per_month_kpef ?? 'N/A' }}</td>-->
                                    <td>{{ $edu->total_marks_cgpa ?? 'N/A' }}</td>
                                    <td>{{ $edu->obtained_marks_cgpa ?? 'N/A' }}</td>
                                    {{-- <td>{{ $edu->currently_studying ?? 'N/A' }}</td> --}}
                                </tr>
                            @endforeach
        ...
        <table cellspacing="0">
            ...
            <tbody>
                @foreach ($data['applicant_fn_r'] ?? [] as $financial)
                    <tr>
                        <td>{{ $financial->name ?? 'N/A' }}</td>
                        <td>{{ $financial->profession ?? 'N/A' }}</td>
                        <td>{{ $financial->financially_supporting ?? 'N/A' }}</td>
                        <td>{{ $financial->relationship ?? 'N/A' }}</td>
                        <td>{{ $financial->gross_income ?? 'N/A' }}</td>
                        <td>{{ $financial->net_income ?? 'N/A' }}</td>
                    </tr>
                @endforeach
    ...
    <div class="table_section">
        ...
        <table cellspacing="0">
            ...
            <tr>
                <td>Net Reusable income (Total monthly income (Subtract -) Total monthly expenditure.)</td>
                <td>{{ $data['expense']->per_month_disposable_income ?? 'Not Available' }}</td>
